# 训练
# 分布式训练+自动混合精度
# CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node 2 --master_port 6036 train_ddp.py --model AdaAtt --id test --batch_size 100 --nproc_per_node 2
...
from data_load import data_load_ddp
...
# data_loader
train_loader = data_load_ddp(config, config.train, 'train')
...
if config.model in ['NIC', 'SAT', 'AdaAtt']:
    optimizer = torch.optim.Adam([{'params': model.module.resnet_encoder.parameters(), 'lr': config.lr_enc},
                                  {'params': model.module.lstm_decoder_att.parameters(), 'lr': config.learning_rate}],
    ...
    for step, (image_feature, cap, cap_len) in enumerate(train_loader):
        ...
        if local_rank == 0:
            if global_step % config.save_model_freq == 0:
                ...
                # 保存模型
                writer.save_model(model.module, global_step)

                # validation
                model.module.eval()
                gen_pycoco_path = generate_captions(config, model.module, global_step, 'val')
